...
class NotificationController extends Controller
{
        ...
        try {
            ...
            switch ($request->canalkey) {
                        ...
                        if (count($files) >= 1) { 
                            ...
                            if (strpos($files[0], '.mp4') !== false) {
                                ...
                                if ($err) {
                                    ...
                                    $data = ["phone" => $interphone, "message" => strip_tags($message->message), "media" => $itemsList];
                                    $curl = curl_init();
                                    curl_setopt_array($curl, [
                                        CURLOPT_URL => "https://api.wassenger.com/v1/messages",
                                        CURLOPT_RETURNTRANSFER => true,
                            ...
                            $data = ["phone" => $interphone, "message" => strip_tags($request->message)];
                            $curl = curl_init();
                            curl_setopt_array($curl, [
                                CURLOPT_URL => "https://api.wassenger.com/v1/messages",
                                CURLOPT_RETURNTRANSFER => true,
                ...
                case "email":
                    ...
                    $path = '/' . Param::getApp() . '/banner' . $signature->logo;
                    ...
                    $data["imagePath"] = $origineurl . $path;
    ...
    public function multiSendAtGroups(SendMessage $request)
    {
        ...
        foreach ($groupes as $groupe) {
            $data = ["group" => $groupe, "message" => $request->message];
        ...
        if (count($notifications) != 0) {
            ...
            foreach ($notifications as $notification) {
                ...
                if (strpos($message->canal, 'email') != false && $notification->canal == 'email') {
                    $signature = $allabonnements->where('user_id', $message->user_id);
                    $file = $signature->pluck('logo')->first();
                    $path = '/' . Param::getApp() . '/banner' . $file;
                    $path = $origineurl .  $path;
                    ...
                    if (count($files) > 0) {
                        ...
                        if (strpos($files, '.mp4') != false) {
                            ...
                            $data = ["phone" => $interphone, "message" => strip_tags($message->message), "media" => ["url" => $url]];
                            $curl = curl_init();
                            curl_setopt_array($curl, [
                                CURLOPT_URL => "https://api.wassenger.com/v1/messages",
                                CURLOPT_RETURNTRANSFER => true,
                        ...
                        sleep(1);
                        $data = ["phone" => $interphone, "message" => strip_tags($message->message)];
                        $curl = curl_init();
                        curl_setopt_array($curl, [
                            CURLOPT_URL => "https://api.wassenger.com/v1/messages",
                            CURLOPT_RETURNTRANSFER => true,
